...
def sort_token_match_search_on_c(token, en_zh_c):
    fl = list(filter(lambda t2: token in t2[1]["zh"], en_zh_c.items()))

    def score_func(d):
        return sum(map(lambda x: x == token, d["zh"])) / (len(d["zh"]) + d.get("en_cnt", 0))

    return sorted(fl, key=lambda t2: score_func(t2[1]), reverse=True)
...
class KBEntityLinker(Component, Serializable):
    ...
    def cut_entity_substr(self, entity: str):
        ...
        word_tokens = jieba.lcut(entity.lower())
    ...
    def sort_found_entities(self, candidate_entities: List[Tuple[int, str, int]],
        ...
        #entity_lang_search_list = sort_token_match_search(entity, self.en_zh_s_group_list)
        entity_lang_search_list = sort_token_match_search_on_c(entity, self.en_zh_c)

        order_list_for_merge = list(map(lambda t2: (str(t2[1][0]), -1 * t2[0]) ,
        enumerate(entity_lang_search_list)))

        order_dict_for_merge = dict(order_list_for_merge)
        ...
        #### refine entities_ratios
        req = []
        min_val = min(order_dict_for_merge.values()) - 1 if order_dict_for_merge else 0
        for entity_num, entity_id, tokens_matched, fuzz_ratio, num_rels in entities_ratios:
            match_sort_idx = order_dict_for_merge.get(entity_id, min_val)
            req.append(
                (entity_num, entity_id, match_sort_idx, tokens_matched, fuzz_ratio, num_rels)
            )

        entities_ratios = req

        #srtd_with_ratios = sorted(entities_ratios, key=lambda x: (x[2], x[3], x[4]), reverse=True)
        srtd_with_ratios = sorted(entities_ratios, key=lambda x: (x[2], x[3], x[4], x[5]), reverse=True)

        print("order_dict_for_merge :")
        print(order_dict_for_merge)
        print("srtd_with_ratios :")
        print(srtd_with_ratios)
        print("-" * 100)

        srtd_with_ratios = list(map(lambda t6: (t6[0], t6[1], t6[3], t6[4], t6[5]), srtd_with_ratios))
